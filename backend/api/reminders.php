<?php
// Palmer Fleet – Upomienky (vozidlá + vodiči)
...
function getReminders(): void {
    ...
    // ── Upomienky vozidiel (z vehicle_deadlines) ─────────────────────────────
    ...
    // ── Upomienky vodičov (z driver_details) ─────────────────────────────────
    ...
    // Načítaj mená vodičov z webdispečink cache (ak existuje) alebo len ID
    $driverNames = [];
    try {
    ...
    $typy = [
        ...
        'certifikat_a1_platnost'    => ['label' => 'Certifikát A1',    'stavField' => 'certifikat_a1_stav'],
        'poistenie_vodica_platnost' => ['label' => 'Poistenie vodiča', 'stavField' => 'poistenie_vodica_stav'],
    ];

    foreach ($driverRows as $row) {
        $wdId = (int)$row['wd_driver_id'];
        $meno = $driverNames[$wdId] ?? "Vodič #$wdId";
...
function updateDriverReminderStav(int $wdDriverId, array $input): void {
    ...
    if (!in_array($field, $allowed)) sendError(400, 'Neplatný stav_field');
    ...
    if ($stmt->rowCount() === 0) sendError(404, 'Vodič nenájdený');

    // Načítaj platnost pre výpočet auto stavu
